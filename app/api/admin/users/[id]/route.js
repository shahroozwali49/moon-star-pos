...
export async function PATCH(request, context) {
  ...
  if (!validId(id)) return Response.json({ error: "Invalid user id" }, { status: 400 });
  ...
  if (body.storeIds !== undefined) {
    if (!Array.isArray(body.storeIds)) return Response.json({ error: "storeIds must be an array" }, { status: 400 });
