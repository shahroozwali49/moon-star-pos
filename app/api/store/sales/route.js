...
export async function POST(request) {
  ...
  const { lines = [], paymentMethod = "CASH", discount = 0 } = body;
  ...
  try {
    ...
    await sessionDb.withTransaction(async () => {
      ...
      const sale = await Sale.create([{
        saleNumber, storeId, cashierId: user._id,
        lines: normalized.map(({item,qty}) => ({ itemId:item._id, name:item.name, sku:item.sku, quantity:qty, unitPrice:item.sellingPrice, lineTotal:item.sellingPrice*qty })),
        subtotal, discount:safeDiscount, total, paymentMethod
      }], { session: sessionDb });
      result = sale[0];
    });
    return Response.json({ sale: { id:String(result._id), saleNumber:result.saleNumber, subtotal:result.subtotal, discount:result.discount, total:result.total, paymentMethod:result.paymentMethod } }, { status:201 });
  } catch (e) {
